//
// Created by Alex Maxime on 2019-01-07.
// Copyright (c) 2019 Trailbehind inc. All rights reserved.
//
...
@implementation OTRRoutingPolylineUtils
static const double precision = 1e-6;

+ (CLLocationCoordinate2D*)decodeLocationPolyline:(NSString*)polyline length:(NSUInteger*)outputLength {
...
+ (OTRGeoPoint*)decodePolyline:(NSString*)polyline length:(NSUInteger*)outputLength {
    ...
    while (idx < length) {
        ...
        coords[coordIdx++] = OTRGeoPointMake(latitude * precision, longitude * precision);
...
+ (NSString*)encodePolylineForCoordinates:(OTRGeoPoint*)coordinates length:(NSUInteger)inputLength {
...
+ (NSString*)encodePolylineForLocations:(CLLocationCoordinate2D*)coordinates length:(NSUInteger)inputLength {
    NSMutableString *output = [[NSMutableString alloc] initWithCapacity:inputLength * 4];
...
+ (NSString*)encodeCoordinate:(int)lat lon:(int)lon {
    return [[OTRRoutingPolylineUtils encodeValue:lat]
            stringByAppendingString:[OTRRoutingPolylineUtils encodeValue:lon]];
}
